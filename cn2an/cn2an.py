...
class Cn2An(object):
    ...
    def __get_pattern(self) -> dict:
        ...
        return {
            "strict": {"int": str_int_pattern, "dec": str_dec_pattern},
            "normal": {"int": nor_int_pattern, "dec": nor_dec_pattern},
        }

    def __copy_num(self, num):
        return "".join(self.conf["number_low_an2cn"][int(n)] for n in num)
    ...
    def __check_input_data_is_valid(self, check_data: str, mode: str) -> (int, str, str, bool):
        ...
        if result := yjf_pattern.search(check_data):
        ...
        if "点" in check_data:
            ...
            if mode == "smart":
                ...
                if result1 := pattern1.search(integer_data):
        ...
        if result_int := re.compile(self.pattern_dict[mode]["int"]).search(
            integer_data
        ):
            if result_int.group() == integer_data:
                if decimal_data is None:
                    return sign, integer_data, decimal_data, False
                if result_dec := re.compile(self.pattern_dict[mode]["dec"]).search(
                    decimal_data
                ):
            ...
            if result_all_num := ptn_all_num.search(integer_data):
                if result_all_num.group() == integer_data:
                    if decimal_data is None:
                        return sign, integer_data, decimal_data, True

                    if result_dec := re.compile(
                        self.pattern_dict[mode]["dec"]
                    ).search(decimal_data):
                        if result_dec.group() == decimal_data:
                            return sign, integer_data, decimal_data, True
            # 口语模式：一万二，两千三，三百四
            ptn_speaking_mode = re.compile(f"^[{self.all_num}][{self.all_unit}][{self.all_num}]$")
            if result_speaking_mode := ptn_speaking_mode.search(integer_data):
